...
// Create client only if we have the required env vars (avoids build-time errors)
...
// Get all admin users
export async function getAllAdminUsers(siteId: string = 'gdp'): Promise<AdminUser[]> {
  const { data, error } = await supabase
    .from('admin_users')
    .select('*')
    .eq('site_id', siteId)
...
export async function getSetting(key: string, siteId: string = 'gdp'): Promise<string | null> {
  const { data, error } = await supabase
    .from('site_settings')
    .select('value')
    .eq('site_id', siteId)
...
export async function getAllSettings(siteId: string = 'gdp'): Promise<Record<string, string>> {
  const { data, error } = await supabase
    .from('site_settings')
    .select('key, value')
    .eq('site_id', siteId);
...
export async function upsertSetting(key: string, value: string, siteId: string = 'gdp'): Promise<boolean> {
  const { error } = await supabase
    .from('site_settings')
    .upsert(
...
export async function upsertSettings(settings: Record<string, string>, siteId: string = 'gdp'): Promise<boolean> {
